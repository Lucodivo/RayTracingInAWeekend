...
public:
  ...
  bool scatter(const Ray& rayIn, const HitRecord& hitRec, Vec3& attenuation, Ray& scatteredRay) const {
    Vec3 target = hitRec.position + hitRec.normal + randomPointInUnitSphere();
...
public:
  ...
  bool scatter(const Ray& rayIn, const HitRecord& hitRec, Vec3& attenuation, Ray& scatteredRay) const {
    ...
    scatteredRay = Ray(hitRec.position, reflected + fuzz * randomPointInUnitSphere());
    attenuation = albedo;
...
public:
  ...
  bool scatter(const Ray &rayIn, const HitRecord &hitRec, Vec3 &attenuation, Ray &scatteredRay) const {
    ...
    if(randomFraction() >= reflectProbability && refract(rayIn.dir(), normal, iorIncomingOverOutgoing, refracted)) {
